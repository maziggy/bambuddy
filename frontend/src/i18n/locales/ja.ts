export default {
  ...
  virtualPrinter: {
    ...
    howItWorks: {
      title: '仕組み',
      intro: '各仮想プリンターはSSDPディスカバリーを通じてBambu Studio / OrcaSlicerに表示されます。異なるモードで複数のプリンターを作成できます:',
      archiveDesc: 'このプリンターに送信された3MFファイルはアーカイブに直接保存されます。',
      reviewDesc: 'ファイルはレビュー用に保留されます。アーカイブ前に各アップロードを承認または破棄します。',
      queueDesc: 'ファイルはアーカイブされ、自動的に印刷キューに追加されます（未割り当て）。',
      proxyDesc: 'トラフィックは実際のプリンターに転送されます。プロキシごとに専用IPが必要です（インターフェースエイリアス）。',
    },
    status: {
      mode: 'モード',
