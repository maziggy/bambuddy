    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-bambu-dark-secondary rounded-xl border border-bambu-dark-tertiary w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option value="null">{t('queue.unassigned')}</option>
              {printers.map(p => (
          ...
          <div>
            ...
            <div className="space-y-2">
              <TriStateToggle label={t('queue.bulkEdit.stagedManualStart')} value={manualStart} onChange={setManualStart} />
              <TriStateToggle label={t('queue.bulkEdit.autoPowerOff')} value={autoOffAfter} onChange={setAutoOffAfter} />
              <TriStateToggle label={t('queue.bulkEdit.requirePreviousSuccess')} value={requirePreviousSuccess} onChange={setRequirePreviousSuccess} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <TriStateToggle label={t('queue.bulkEdit.bedLevelling')} value={bedLevelling} onChange={setBedLevelling} />
              <TriStateToggle label={t('queue.bulkEdit.flowCalibration')} value={flowCali} onChange={setFlowCali} />
              <TriStateToggle label={t('queue.bulkEdit.vibrationCalibration')} value={vibrationCali} onChange={setVibrationCali} />
              <TriStateToggle label={t('queue.bulkEdit.firstLayerInspection')} value={layerInspect} onChange={setLayerInspect} />
              <TriStateToggle label={t('queue.bulkEdit.timelapse')} value={timelapse} onChange={setTimelapse} />
              <TriStateToggle label={t('queue.bulkEdit.useAms')} value={useAms} onChange={setUseAms} />
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 p-4 border-t border-bambu-dark-tertiary">
          ...
          >
            {isSaving ? t('queue.bulkEdit.saving') : t('queue.bulkEdit.applyChanges')}
          </Button>
        </div>
      </div>
    </div>
            ...
            <span className={`flex items-center gap-1.5 ${item.printer_id === null && !item.target_model ? 'text-orange-400' : ''} ${item.target_model ? 'text-blue-400' : ''}`}>
              ...
              {item.target_model
                ? t('queue.anyModel', {
                    ...
                    filaments: item.required_filament_types?.length ? ` (${item.required_filament_types.join(', ')})` : ''
                  })
                : item.printer_id === null
                  ? t('queue.unassigned')
                  : (item.printer_name || t('queue.printerNumber', { id: item.printer_id }))}
            </span>
          ...
          {isPending && (
            <>
              ...
              <Button
                ...
                title={!hasPermission('queue:update') ? t('queue.permissions.noEditPermission') : t('common.edit')}
              >
              ...
              <Button
                ...
                title={!hasPermission('queue:delete') ? t('queue.permissions.noCancelPermission') : t('common.cancel')}
                className="text-red-400 hover:text-red-300 hover:bg-red-500/10"
              >
                <X className="w-4 h-4" />
              </Button>
          ...
          {isHistory && (
            <>
              ...
              <Button
                ...
                onClick={onRemove}
                disabled={!hasPermission('queue:delete')}
                title={!hasPermission('queue:delete') ? t('queue.permissions.noRemovePermission') : t('queue.remove')}
              >
...
export function QueuePage() {
  ...
  const bulkUpdateMutation = useMutation({
    ...
    onError: () => showToast(t('queue.toast.failedToUpdateItems'), 'error'),
  });
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        {historyItems.length > 0 && (
          <Button
            ...
            disabled={!hasPermission('queue:delete')}
            title={!hasPermission('queue:delete') ? t('queue.permissions.noClearHistoryPermission') : undefined}
          >
            <Trash2 className="w-4 h-4" />
                ...
                {selectedItems.length > 0 && (
                  <>
                    ...
                    <Button
                      ...
                      disabled={!hasPermission('queue:update')}
                      title={!hasPermission('queue:update') ? t('queue.permissions.noEditPermission') : undefined}
                    >
                      <Pencil className="w-4 h-4" />
                    ...
                    <Button
                      ...
                      title={!hasPermission('queue:delete') ? t('queue.permissions.noCancelPermission') : undefined}
                    >
                      <X className="w-4 h-4" />
