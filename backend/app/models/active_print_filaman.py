"""Track FilaMan data for active prints."""
...
class ActivePrintFilaMan(Base):
    """Stores FilaMan tracking data for active prints.

    Captured at print start and used at print completion to report
    per-filament consumption to FilaMan via POST /api/v1/spools/{id}/consumptions.
    Rows are deleted after the print completes.

    Key: (printer_id, archive_id) — allows same archive on different printers.
    """

    __tablename__ = "active_print_filaman"
    __table_args__ = (UniqueConstraint("printer_id", "archive_id", name="uq_filaman_printer_archive"),)
    ...
    # tray_uuid is used to look up the corresponding FilaMan spool via rfid_uid
