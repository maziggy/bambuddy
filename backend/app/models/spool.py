...
class Spool(Base):
    ...
    tag_uid: Mapped[str | None] = mapped_column(String(32))  # Primary RFID tag UID (up to 32 hex chars)
    tag_uid_2: Mapped[str | None] = mapped_column(
        String(32)
    )  # Secondary RFID tag UID — same physical spool, different reader hardware
