...
- **P2S Camera Stream Disconnects After a Few Seconds** ([#661](https://github.com/maziggy/bambuddy/issues/661)) — The P2S firmware drops RTSP sessions after a few seconds with an I/O error. The backend treated this as a fatal failure, ending the MJPEG stream and forcing the frontend through a full reconnection cycle (stop → start → brief connection → fail → repeat). Multi-part fix: (1) Added transparent auto-reconnection — when ffmpeg's RTSP connection dies, it respawns immediately and continues streaming without interruption. (2) Added GnuTLS TLS compatibility config (`%UNSAFE_RENEGOTIATION`, `%COMPAT`, SHA-1 override) to the Docker image — ffmpeg uses GnuTLS (not OpenSSL), and the hardened Debian defaults may reject the P2S firmware's TLS renegotiation, causing the session drops. (3) Optimized ffmpeg startup with fast-start flags (`-probesize 32 -analyzeduration 0 -fflags nobuffer -flags low_delay`) and reduced reconnect delay from 1.0s to 0.2s, cutting per-cycle overhead from ~3-4s to under 1s. (4) Added ffmpeg error resilience (`-err_detect ignore_err -fflags +discardcorrupt+genpts`) so transient stream errors don't abort the session. (5) Fixed external camera streams (e.g. go2rtc) being choppy due to double rate-limiting in the proxy layer, and applied the same fast-start/error-resilience flags and auto-reconnect (30 retries) to external RTSP streams. (6) Added `rtsps://` to allowed external camera URL schemes. GnuTLS debug logging is now included in support packages when debug mode is enabled. Reported by @ddetton, confirmed by @DMoenning.
