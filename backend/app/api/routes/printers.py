...
async def get_printer_status(
    ...
    # Inject HA enclosure sensor readings when printer has them configured.
    # Only fills chamber/humidity if the printer hasn't already provided them via MQTT.
    enclosure = homeassistant_service.get_cached_enclosure(printer_id)
    if enclosure:
        temperatures = dict(temperatures)
        if enclosure.get("temp") is not None and temperatures.get("chamber") is None:
            temperatures["chamber"] = enclosure["temp"]
            temperatures["enclosure_temp_unit"] = enclosure.get("temp_unit", "°C")
        if enclosure.get("humidity") is not None:
            temperatures["enclosure_humidity"] = enclosure["humidity"]
            temperatures["enclosure_humidity_unit"] = enclosure.get("humidity_unit", "%")
        if enclosure.get("fan_on") is not None:
            temperatures["enclosure_fan_on"] = enclosure["fan_on"]
