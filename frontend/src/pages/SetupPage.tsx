  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (authEnabled) {
      ...
      if (adminUsername || adminPassword) {
        if (!adminUsername || !adminPassword) {
          showToast(t('setup.toast.enterBothOrLeaveEmpty'), 'error');
          return;
        }
        ...
        if (adminPassword.length < 6) {
          showToast(t('setup.toast.passwordMinLength'), 'error');
          return;
        }
      }
    }
              ...
              <div className="space-y-4 mt-4">
                <div className="p-3 bg-bambu-dark-secondary/50 border border-bambu-dark-tertiary rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-bambu-gray">
                      ...
                      <p>
                        {t('setup.adminAccountDescription')}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <label htmlFor="admin-username" className="block text-sm font-medium text-white mb-2">
                    {t('setup.adminUsername')} <span className="text-bambu-gray text-xs">({t('setup.optionalIfAdminsExist')})</span>
                  </label>
                  <input
                    id="admin-username"
                    type="text"
                    ...
                    placeholder={t('setup.placeholderAdminUsername')}
                ...
                <div>
                  <label htmlFor="admin-password" className="block text-sm font-medium text-white mb-2">
                    {t('setup.adminPassword')} <span className="text-bambu-gray text-xs">({t('setup.optionalIfAdminsExist')})</span>
                  </label>
                  <input
                    id="admin-password"
                    type="password"
                    ...
                    className="block w-full px-4 py-3 bg-bambu-dark-secondary border border-bambu-dark-tertiary rounded-lg text-white placeholder-bambu-gray focus:outline-none focus:ring-2 focus:ring-bambu-green/50 focus:border-bambu-green transition-colors"
                    placeholder={t('setup.placeholderAdminPassword')}
                    minLength={6}
